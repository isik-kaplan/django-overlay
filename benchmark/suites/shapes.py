...
def _report(ctx, section, label, view_sql, plain_sql, notes=""):
    ...
    section.add(
        label,
        {"overlay": harness.Cell(view_ms, capped=view_timeout),
         "plain": harness.Cell(plain_ms)},
        ratio=ratio, plan=shape, notes=notes,
    )


def _relation_sizes():
    section = harness.Section(
        "What was loaded", ("view", "base", "source", "shape"),
    )
...
def run(ctx):
    ...
    section = harness.Section(
        "A. Entity shape (overridable + soft_delete) -- the expensive half", COLUMNS)
    _report(ctx, section, "point lookup by pk",
    ...
    _report(ctx, section, "count(*)",
            "SELECT count(*) FROM bench_person_view",
            f"SELECT count(*) FROM {plain['person']}")
    yield section

    section = harness.Section(
        "B. Link shape (overridable=False, soft_delete=False) -- the cheap half", COLUMNS)
    _report(ctx, section, "person_id lookup",
    ...
    _report(ctx, section, "detail: one person -> addresses",
    ...
    _report(ctx, section, "detail: one person -> all three relations",
    ...
    _report(ctx, section, "reverse: people at addresses in a city",
    ...
    _report(ctx, section, "reverse, rewritten as = ANY (ARRAY ...)",
    ...
    _report(ctx, section, "reverse: people with a phone number",
    ...
    _report(ctx, section, "list page: 20 people + their address count",
    ...
    section = harness.Section(
        "D. The detail page, three ways -- can the gap be written away?", COLUMNS)
    _report(ctx, section, "1. JOIN through the link view",
            "SELECT a.* FROM bench_address_view a "
            "JOIN bench_person_address_view l ON l.address_id = a.id "
            f"WHERE l.person_id = {one_person}",
            f"SELECT a.* FROM {plain['address']} a "
            f"JOIN {plain['person_address']} l ON l.address_id = a.id "
            f"WHERE l.person_id = {one_plain}")
    _report(ctx, section, "2. = ANY (ARRAY (subquery))",
    ...
    link_ids = [
        str(row[0]) for row in graph.rows(
    ...
    _report(ctx, section, "3. two queries, literal id list (prefetch_related)",
    ...
    _report(ctx, section, "phone lookup, JOIN form",
            "SELECT person.id FROM bench_person_view person "
            "JOIN bench_person_phone_view l ON l.person_id = person.id "
            "JOIN bench_phone_view p ON p.id = l.phone_id "
            "WHERE p.number = '+447000000042' LIMIT 50",
            ...
            f"JOIN {plain['phone']} p ON p.id = l.phone_id "
            f"WHERE p.number = '+447000000042' LIMIT 50")
    _report(ctx, section, "phone lookup, = ANY (ARRAY ...)",
            "SELECT person.id FROM bench_person_view person WHERE person.id = ANY (ARRAY("
